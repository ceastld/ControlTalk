...
class Vgg19(torch.nn.Module):
    ...
    def __init__(self, requires_grad=False):
        ...
        vgg_pretrained_features = models.vgg19(weights=models.VGG19_Weights.DEFAULT).features # pretrained=True
...
class Transform:
    ...
    def __init__(self, bs, **kwargs):
        noise = torch.normal(mean=0, std=kwargs['sigma_affine'] * torch.ones([bs, 2, 3])) # 0.05
        self.theta = noise + torch.eye(2, 3).view(1, 2, 3)
        self.bs = bs

        if ('sigma_tps' in kwargs) and ('points_tps' in kwargs): # 0.005 5
            self.tps = True
...
class GeneratorFullModel(torch.nn.Module):
    ...
    def __init__(self, kp_extractor, he_estimator, generator, discriminator, train_params, estimate_jacobian=True):
        ...
        if self.loss_weights['headpose'] != 0:
            ...
            hopenet_state_dict = torch.load(train_params['hopenet_snapshot'], map_location=torch.device('cpu'))
            self.hopenet.load_state_dict(hopenet_state_dict)
            if torch.cuda.is_available():
                self.hopenet = self.hopenet.cuda()# .half()
                self.hopenet.eval()
                print('Hopenet moved to GPU and set to eval')

    def forward(self, x, clip_train=False):
        ...
        if self.loss_weights['generator_gan'] != 0:
            if clip_train:
                clip_len = 5
                batch_size = x['driving'].shape[0] // clip_len
                for k,v in pyramide_generated.items():
                    pyramide_generated[k] = torch.stack(torch.split(v, split_size_or_sections=clip_len, dim=0), dim=0).view(batch_size, -1, v.shape[-2], v.shape[-1])
                for k,v in pyramide_real.items():
                    pyramide_real[k] = torch.stack(torch.split(v, split_size_or_sections=clip_len, dim=0), dim=0).view(batch_size, -1, v.shape[-2], v.shape[-1])
            ...
            for scale in self.disc_scales:
                ...
                map_gen_at_key = discriminator_maps_generated[key]
                bs, _, h, w = map_gen_at_key.shape
                mouth_map_gen_at_key = map_gen_at_key[:, :, h // 2 - h * 0//8 : h // 2 + (h*4)//8, w // 2 - w // 4 : w // 2 + w // 4]
                if self.train_params['gan_mode'] == 'hinge':
                    
                    value = 1 - torch.mean(map_gen_at_key)
                    value_mouth = 1 - torch.mean(mouth_map_gen_at_key)
                elif self.train_params['gan_mode'] == 'ls':
                    value = ((1 - map_gen_at_key) ** 2).mean()
                    value_mouth = ((1 - mouth_map_gen_at_key) ** 2).mean()
                else:
                    raise ValueError('Unexpected gan_mode {}'.format(self.train_params['gan_mode']))

                value_total += self.loss_weights['generator_gan'] * (0.333*value + (1-0.333)*value_mouth)
            loss_values['gen_gan'] = value_total / len(self.disc_scales)
            ...
            if sum(self.loss_weights['feature_matching']) != 0:
                ...
                for scale in self.disc_scales:
                    ...
                    for i, (a, b) in enumerate(zip(discriminator_maps_real[key], discriminator_maps_generated[key])):
                        ...
                        bs, _, h, w = a.shape
                        a_mouth = a[:, :, h // 2 - h * 0//8 : h // 2 + (h*4)//8, w // 2 - w // 4 : w // 2 + w // 4]
                        b_mouth = b[:, :, h // 2 - h * 0//8 : h // 2 + (h*4)//8, w // 2 - w // 4 : w // 2 + w // 4]
                        value_mouth = torch.abs(a_mouth - b_mouth).mean()
                        value_total += self.loss_weights['feature_matching'][i] * (0.333*value + (1-0.333)*value_mouth)
                loss_values['feature_matching'] = value_total / len(self.disc_scales)
...
class DiscriminatorFullModel(torch.nn.Module):
    ...
    def forward(self, x, generated, clip_train=False):
        pyramide_real = self.pyramid(x['driving'])
        ...
        for scale in self.scales:
            key = 'prediction_map_%s' % scale
            map_real_at_key = discriminator_maps_real[key]
            map_gen_at_key = discriminator_maps_generated[key]
            # 
            bs, _, h, w = map_real_at_key.shape
            mouth_map_real_at_key = map_real_at_key[:, :, h // 2 - h * 0//8 : h // 2 + (h*4)//8, w // 2 - w // 4 : w // 2 + w // 4]
            bs, _, h, w = map_gen_at_key.shape
            mouth_map_gen_at_key = map_gen_at_key[:, :, h // 2 - h * 0//8 : h // 2 + (h*4)//8, w // 2 - w // 4 : w // 2 + w // 4]
            if self.train_params['gan_mode'] == 'hinge':
                value = -torch.mean(torch.min(map_real_at_key-1,       self.get_zero_tensor(map_real_at_key)))  \
                       - torch.mean(torch.min(-map_gen_at_key-1, self.get_zero_tensor(map_gen_at_key)))
                value_mouth = -torch.mean(torch.min(mouth_map_real_at_key-1,       self.get_zero_tensor(mouth_map_real_at_key)))  \
                          - torch.mean(torch.min(-mouth_map_gen_at_key-1, self.get_zero_tensor(mouth_map_gen_at_key)))
            elif self.train_params['gan_mode'] == 'ls':
                value = ((1 - map_real_at_key) ** 2 + map_gen_at_key ** 2).mean()
                value_mouth = ((1 - mouth_map_real_at_key) ** 2 + mouth_map_gen_at_key ** 2).mean()
            else:
                raise ValueError('Unexpected gan_mode {}'.format(self.train_params['gan_mode']))

            value_total += self.loss_weights['discriminator_gan'] * (0.333*value + (1-0.333)*value_mouth)
        loss_values['disc_gan'] = value_total / len(self.scales)
